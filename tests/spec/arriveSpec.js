...
describe("Leave Event Tests.", function() {
  ...
  describe("Leave event called without selector", function() {
    it("The leave event should be fired when the element on which leave was called is removed from DOM.", function (done) {
      var $toBeRemoved = $("<div class='test-elem'></div>");

      $("body").append($toBeRemoved);

      j(document).unbindLeave();

      j($toBeRemoved).leave(function () {
        expect(this).toBe($toBeRemoved[0]);
        done();
      });
    ...
    it("The leave event should be fired when parent element of the element on which leave was called is removed from DOM.", function (done) {
      var $elem = $("<div class='test-elem'><div class='test-elem-2'></div></div>");

      $("body").append($elem);

      $toBeRemoved = j(".test-elem-2");

      j(document).unbindLeave();

      $toBeRemoved.leave(function () {
        expect(this).toBe($toBeRemoved[0]);
        done();
      });
...
describe("Calling arrive/leave functions on NodeList and HTMLElement.", function() {
  ...
  it("Leave function should be callable on NodeList.", function() {
    document.getElementsByTagName("body").leave(".test", function() {});
    expect(true).toBeTruthy();
  });

  it("Leave function should be callable on HTMLElement.", function() {
    document.getElementsByTagName("body")[0].leave(".test", function() {});
    expect(true).toBeTruthy();
  });
});
