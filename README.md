...
var options = {
    ...
    existing: boolean                      // Defaults to false. Setting it to true would ensure that the registered callback fire for the elements already existing in the DOM and matching the selector. If options.onceOnly is set, the callback is only called once with the first element matching the selector before being unbound.
};
```
Example:
```javascript
